...
public class ENEMY_MOVEMENT4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        zombie4 = GetComponent<NavMeshAgent>();
    ...
    void Update()
    {
        ...
        attack_radius = zombie4.stoppingDistance;
        ...
        if (Vector3. Distance(Target_points[currentTransformIndex].position, transform.position) <= zombie4.stoppingDistance + 1.5f && !isChasing)
        {
            zombie_Patrol();         
            isPatroling = true;
        }
        ...
        if (Distance(Player_pos, transform) <= zombie4.stoppingDistance)
        {
            isAttacking = true;
        }
        else if(Distance(Player_pos, transform) >= zombie4.stoppingDistance)
        {
            isAttacking = false;
        }
    }
    ...
    public void follow_player()
    {
        ...
        isPatroling = false;
        zombie4.destination = Player_pos.position;
    }
    public void zombie_Patrol()
    {
        int targetID = Random.Range(0, Target_points.Length);
        zombie4.speed = 2;
        zombie4.SetDestination(Target_points[targetID].position);
        zombie4.transform.LookAt(Target_points[targetID].position);
